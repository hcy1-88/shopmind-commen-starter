...
public class TraceIdInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        String traceId = request.getHeader(ShopmindHeaderConstant.TRACE_ID_HEADER);
        if (StrUtil.isBlank(traceId)) {
            traceId = TraceIdUtils.generateTraceId();
        }

        // 将 TraceId 放入 MDC（用于日志打印）
        MDC.put(ShopmindHeaderConstant.TRACE_ID_HEADER, traceId);

        // 将 TraceId 放入响应头（方便前端追踪）
        response.setHeader(ShopmindHeaderConstant.TRACE_ID_HEADER, traceId);
    ...
    public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex) {
        ...
        MDC.remove(ShopmindHeaderConstant.TRACE_ID_HEADER);
        UserContext.clear();
    }
}
